    ...
    <div class="page-content header-clear-small">
      <div class="card card-style">
        <div class="content">
          ...
          >
            <h3 class="bold top-10">暫時沒有這歌手資料</h3>
...
if (singerlist.value) {
  showresult.value = [...singerlist.value];
} else {
  navigateTo("/");
}

async function choosesinger(singername) {
  loading.value = true;
  let result = await useFetch(
    "https://7aof7x0rkc.execute-api.us-east-1.amazonaws.com/getartistsongs?name=" +
      singername
  );

  singersonglist.value = result.data.value.body;
  navigateTo("/singersongs#" + singername);
}
